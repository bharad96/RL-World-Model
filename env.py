...
#         super(EnduroMDNRNN, self).__init__(env, full_episode=full_episode)
#         self.with_obs = with_obs  # whether or not to return the frame with the encodings
#         self.vae = CVAE(args)
#         self.rnn = MDNRNN(args)
#
...
#         # whats happening?
#         self.rnn_states = rnn_init_state(self.rnn)
#         # how does the super class know about z_state?
#         if self.with_obs:
#             [z_state, obs] = super(EnduroMDNRNN, self).reset()  # calls step
#             return [z_state, obs]
#         else:
#             z_state = super(EnduroMDNRNN, self).reset()  # calls step
#             return z_state
#
#     def step(self, action):
#         obs, reward, done, _ = super(EnduroMDNRNN, self).step(action)
#         z = tf.squeeze(self.encode_obs(obs))
#         h = tf.squeeze(self.rnn_states[0])
#         c = tf.squeeze(self.rnn_states[1])
#         if self.rnn.args.state_space == 2:
...
def make_env(args, dream_env=False, seed=-1, render_mode=False, full_episode=False, with_obs=False, load_model=True):
    ...
    #         env = EnduroMDNRNN(args, gym.make('Enduro-v0').env, load_model, full_episode, with_obs)
