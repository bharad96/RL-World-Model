...
def ds_gen():
    filenames = os.listdir(DATA_DIR)[:1000] # only use first 10k episodes
    n = len(filenames)
    for j, fname in enumerate(filenames):
        ...
        n_pad = args.max_frames - img.shape[0] # pad so they are all 5 thousand step long episodes
        img = tf.pad(img, [[0, n_pad], [0, 0], [0, 0], [0, 0]])
        action = tf.pad(action, [[0, n_pad], [0, 0]])
        reward = tf.pad(reward, [[0, n_pad]])
        yield img, action, reward

def create_tf_dataset():
    dataset = tf.data.Dataset.from_generator(ds_gen, output_types=(tf.float32, tf.float32, tf.float32), output_shapes=((args.max_frames, 64, 64, 3), (args.max_frames, args.a_width), (args.max_frames,)))
    return dataset
...
for batch in dataset:
  i += 1
  obs_batch, action_batch, r = batch
...
np.savez_compressed(os.path.join(SERIES_DIR, "series.npz"), action=action_dataset, mu=mu_dataset, logvar=logvar_dataset, reward=r_dataset)
print("saved series.npz")